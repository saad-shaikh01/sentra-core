...
@Injectable()
export class HrmsCacheService {
  private readonly defaultTtl: number;
  private readonly logger = new Logger(HrmsCacheService.name);
  ...
  buildKey(organizationId: string, resource: string, ...parts: string[]): string {
    return ['hrms', organizationId, resource, ...parts].join(':');
  }
  ...
  async invalidateOrgResource(organizationId: string, resource: string): Promise<void> {
    await this.delByPrefix(`hrms:${organizationId}:${resource}`);
  }
