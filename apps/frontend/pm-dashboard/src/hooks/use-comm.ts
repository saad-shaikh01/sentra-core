...
export function useProjectClientEmail(projectId: string) {
  return useQuery({
    queryKey: ['project-client-email', projectId],
    queryFn: async () => {
      const projectRes = await api.getProject(projectId);
      const project = projectRes?.data;
      if (!project?.clientId) return null;
      const client = await api.getClient(project.clientId);
      return (client?.email ?? client?.data?.email ?? null) as string | null;
    },
    enabled: !!projectId,
  });
}
