/**
 * backfill-frontsell-sales.ts
 *
 * Targets ALL Sales where saleType = 'FRONTSELL' (not deleted).
 *
 * Updates:
 ...
 *     libs/backend/prisma-client/prisma/scripts/backfill-frontsell-sales.ts
 */
...
async function main(): Promise<void> {
  ...
  console.log(`Sales (FRONTSELL)          : ${Number(salesCount!.count)}`);
  ...
  if (Number(salesCount!.count) === 0) {
    console.log('\nNo FRONTSELL sales found. Nothing to do.');
    return;
  }
